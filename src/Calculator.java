...
public abstract class Calculator<D extends Number> {
    // 제네릭을 사용하도록 교체 D: Double를 기본적으로 사용한다는 뜻
    ...
    public void CheckNumber(double d) {
        ...
        boolean checkValue = getList().stream().anyMatch(list -> list.doubleValue() > d);

        if(checkValue) {
            getList().stream().filter(list -> list.doubleValue() > d)
    ...
    // (공통)조회 메서드
    public ArrayList<D> inquiryResults() {
        return getList();
    }
